...
class Authorization extends Loggable {
    ...
    basicValidate(req, res, next) {
        if(this.config.authorization.basicDatabase) return this.basicDatabase(req, res, next);
    ...
    async basicValidate(req, res, next) {
        const { username, password } = req.authorization.basic;
        const module = req.headers['x-authorization-module'];
        const connection = database.connections.mongodb[this.config.authorization.basicDatabase.connectionName] || 'application';
        const model      = connection.models.SNFAuthorization || connection.model('SNFAuthorization', new database.mongoose.Schema({
            user: String,
            password: String,
            module: String,
            lastLogin: Date
            ...
            autoCreate: true,
            collection: this.config.authorization.basicDatabase.collectionName || 'snf-authorization'
        }));

        const user  = await model.findOne({ user: username, password, module });
