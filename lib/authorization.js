...
class Authorization extends Loggable {
    ...
    basicValidate(req, res, next) {
        ...
        if (isValidUser) {
            ...
            this.logger.bunyanLogger.fields.user = req.user; // TODO: depois que testar remover essa experiencia
            next();
        } else {
    ...
    bearerValidate(req, res, next) {
        ...
        this.jwt.verify(key, this.config.authorization.jwt.secret, (err, decoded) => {
            if (err) {
                ...
                req.user = decoded;
                this.logger.bunyanLogger.fields.user = JSON.stringify(req.user); // TODO: depois que testar remover essa experiencia
                next();
            }
        });
    }
