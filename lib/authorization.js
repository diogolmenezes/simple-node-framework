...
class Authorization extends Loggable {
        ...
        if (isEnabled) {
            ...
            if (hasSentAuthorization) {
                switch (this.config.authorization.scheme) {
                    ...
                    default:
                        return next(this.applicationErrors.throw('Authorization sheme defined in configuration file is not valid', 'UnauthorizedError')); // 401
                }
            } else {
                res.header('WWW-Authenticate', `${this.config.authorization.scheme} realm='Get API'`);
                return next(this.applicationErrors.throw('You did not sent the authorization header', 'UnauthorizedError')); // 401
            }
        } else {
            return next();
        }
    ...
    basicValidate(req, res, next) {
        ...
        if (username === this.config.authorization.username && password === this.config.authorization.password) {
        ...
        this.jwt.verify(key, this.config.authorization.jwt.secret, (err, decoded) => {
            if (err) {
                if (err.name === 'TokenExpiredError') {
                    next(this.applicationErrors.throw('Expired JWT.', 'ForbiddenError')); // 403
                } else {
                    next(this.applicationErrors.throw('Invalid JWT', 'UnauthorizedError')); // 401
                }
            } else {
