...
class Redis extends Loggable {
    ...
    delPattern(pattern) {
        ...
        if (this.config.mock) {
            Object.keys(this.config.mock).map((key) => {
    ...
    get(key) {
        ...
        if (this.config.mock) {
            ...
            return Promise.reject(new Error(`There is no redis key for [${key}]`));
        }
                ...
                if (!error) {
                    if (data) {
                        try {
                            ...
                            this.log.error(`Parse error for redis key [${key}]`, error);
                            reject(error);
                        }
                    } else {
                        reject(new Error(`There is no redis key for [${key}]`));
                    }
                } else {
