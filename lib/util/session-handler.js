...
class SessionHandler extends Loggable {
    ...
    new(key, ttl) {
        return new Promise((resolve, reject) => {
            ...
            this.data = {};
            var redisKey = this._generateRedisKey(key);

            const sessionDuration = ttl || this.config.ttl;
            this.redis.set(redisKey, this.data, sessionDuration)
                .then((data) => {
                    // Ativa a sessão
                    this.activate();
                    resolve(redisKey);
    ...
    start(req, res, next) {
        ...
        if ('resolveKey' in this.config) {
            ...
            if (Array.isArray(this.config.headers)) {
                for (var k of this.config.headers) {
                    if (k in req.headers) {
    ...
    load(key) {
        ...
        return this.redis.get(this._generateRedisKey(this.key))
            .then((sessionData) => {
    ...
    update() {
        return new Promise((resolve, reject) => {
            if (!this.key) reject(this.gerarErroSessaoNaoIniciada());
                ...
                const redisKey = this._generateRedisKey(this.key);
                this.redis.update(redisKey, this.data, ttl)
                    .then((result) => {
                        resolve(result);
                    })
                    .catch((err) => {
    ...
    _generateRedisKey(key) {
        var prefix = this.config.prefix || 'sess';
        return prefix + ':' + key;
    }
    ...
    gerarErroSessaoNaoIniciada() {
        var error = new Error('Nenhuma sessão inicializada');
