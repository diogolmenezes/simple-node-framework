...
class RequestAndResponseLogger {
    ...
    static request(req, res, next) {
        if (!RequestAndResponseLogger.ignoreUrl(req.url)) {
            ...
            const request = `REQUEST ${requestId} - ${req.method} ${req.url} - Ip [${requestIp}] - Body [${JSON.stringify(req.body)}] - Header [${JSON.stringify(req.headers)}]`;
            req.log.debug('RequestAndResponseLogger', request, { natural: { requestId, requestIp } });
        }
        return next();
    }
    ...
    static response(req, res, route) {
        if (!RequestAndResponseLogger.ignoreUrl(req.url)) {
            ...
            const response = `RESPONSE ${requestId} - ${req.method} ${req.url} - Version [${versions}] - Status [${res.statusCode}] - Ip [${requestIp}] - Body [${data}] - Header [${res._header}]`;
            req.log.debug('RequestAndResponseLogger', response, { natural: { responseStatusCode: String(res.statusCode), requestIp, requestId } });
        }
    }
