...
class RequestAndResponseLogger {
    ...
    static request(req, res, next) {
        if (!RequestAndResponseLogger.ignoreUrl(req.url)) {
            ...
            req.log.debug('RequestAndResponseLogger', request, { natural: { requestId, requestIp, user, requestPath } });
        }
        return next();
    }
    ...
    static response(req, res, route) {
        if (!RequestAndResponseLogger.ignoreUrl(req.url)) {
            ...
            req.log.debug('RequestAndResponseLogger', response, { natural: { responseStatusCode: String(res.statusCode), requestIp, requestId, user, requestPath } });
        }
    }
