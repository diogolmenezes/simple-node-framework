...
class Origin {
    ...
    proccess(req, res, next) {
        if (this.config.origin && this.itsNotToIgnore(req)) {
            ...
            if (!this.hasApplication(req)) {
                return next(
                    this.applicationErrors.throw(
                        `The header x-origin-application is required for request [${requestId}].`,
            ...
            if (!this.hasChannel(req)) {
                return next(
                    this.applicationErrors.throw(
                        `The header x-origin-channel is required for request [${requestId}].`,
                        "PreconditionFailedError"
                    )
                ); // 412
            }
            ...
            if (!this.hasDevice(req)) {
                return next(
                    this.applicationErrors.throw(
                        `The header x-origin-device is required for request [${requestId}].`,
                        "PreconditionFailedError"
                    )
                ); // 412
            }
    ...
    hasApplication(req) {
        return this.config.origin.require.application
            ? req.header("x-origin-application")
            : true;
    }

    // test if request has x-origin-channel header
    hasChannel(req) {
        return this.config.origin.require.channel
            ? req.header("x-origin-channel")
            : true;
    }

    // test if request has x-origin-device header
    hasDevice(req) {
        return this.config.origin.require.device
            ? req.header("x-origin-device")
            : true;
    }
}
