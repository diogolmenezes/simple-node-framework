    ...
    connect() {
        ...
        Object.keys(this.config.db).map(property => {
            ...
            connection.on('connected', () => {
                this.log.debug(`Connected at database [${property}] [${config.url}]`);
                this.connections[property] = connection;
            });

            connection.on('close', () => {
                this.log.debug(`Disconected from database [${property}] [${config.url}]`);
            });

            connection.on('error', (erro) => {
                this.log.error(`Database connection error [${property}] [${config.url}]`, erro);
            });
        });
    }
}
